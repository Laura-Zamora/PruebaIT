<?php
    ...
    $SQL = "SELECT R.ID AS REGISTRO,CONCAT(CONCAT(C.ID,' - '),C.NOMBRE) AS CONDUCTOR,CONCAT(CONCAT(U.ID,' - '),U.NOMBRE_RUTA) AS RUTA,R.HORAINICIO AS HORAI,R.HORAFINAL AS HORAF, R.MINUTOSTOTAL AS TOTAL
            ...
            WHERE R.MINUTOSTOTAL = ( SELECT MAX( MINUTOSTOTAL ) FROM registro WHERE IDCONDUCTOR= $_SESSION[usuario]) OR R.MINUTOSTOTAL = ( SELECT MIN( MINUTOSTOTAL ) FROM registro WHERE IDCONDUCTOR = $_SESSION[usuario]);";
